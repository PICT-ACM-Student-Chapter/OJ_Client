...
const Routes = (props) => {
    ...
    React.useEffect(() => {
        ...
        if (
            ...
            checkContest(match.params.contestId) === 0
        ) {
            addRestrictions(match.params.contestId);
            // eslint-disable-next-line
            prevCont = match.params.contestId;
        } else removeRestrictions(prevCont);
        // eslint-disable-next-line
    ...
    const checkContest = (contestId) => {
        ...
        for (let c of globalContext.contests) {
            ...
            //     return -10
            if (
                c.contest_id?.id === contestId &&
                new Date(c.contest_id.end_time) < new Date()
            )
                return 1;
            if (
                c.contest_id?.id === contestId &&
                new Date(c.contest_id.start_time) > new Date()
            )
                return -1;
            if (c.contest_id?.id === contestId) return 0;
        }
        return -2;
    };
                        ...
                        {match &&
                            (contestStatus === 0 ? (
                                <>
                                    ...
                                    <Route
                                        exact
                                        path="/contests/:contestId/:questionId/submissions"
